...
class ProcessQG(common_base.CommonBase):
    ...
    def initialize_config(self):
        ...
        if self.dataset_type == 'Zjet':
            self.Z_jet_dir = config['Z_jet_dir']
            self.QCD_jet_dir = config['QCD_jet_dir']
            self.Z_jet_filepattern = config['Z_jet_filepattern']
            self.QCD_jet_filepattern_quark = config['QCD_jet_filepattern_quark']
            self.QCD_jet_filepattern_gluon = config['QCD_jet_filepattern_gluon']
    ...
    def load_dataset(self):
        ...
        if self.dataset_type == 'qg':
            ...
            input_files = [os.path.join(self.Z_jet_dir, filename) for filename in os.listdir(self.Z_jet_dir) if self.Z_jet_filepattern in filename]
            input_files += [os.path.join(self.QCD_jet_dir, filename) for filename in os.listdir(self.QCD_jet_dir) if self.QCD_jet_filepattern_quark in filename or self.QCD_jet_filepattern_gluon in filename]
            ...
            for i,input_file in enumerate(input_files):
                ...
                self.df_fjparticles = pd.concat([self.df_fjparticles, df_fjparticles])
                print(f'  n_jets: {self.df_fjparticles.size}')

                # Check if jet pt is in expected range
                #jet_pt_list = [np.sum(np.array([particle.pt() for particle in jet])) for jet in df_fjparticles]
                #if any(jet_pt < self.pt[0] for jet_pt in jet_pt_list) or any(jet_pt > self.pt[1] for jet_pt in jet_pt_list):
                #    print(f'ERROR: jet found with pt outside of expected range')
                #    for jet_pt in jet_pt_list:
                #        if jet_pt < self.pt[0] or jet_pt > self.pt[1]:
                #            sys.exit(jet_pt)

                # Set labels (Z=1, QCD=0)
                if '_Zjet_' in input_file:
                    y = np.ones(len(df_fjparticles))
                else:
                    y = np.zeros(len(df_fjparticles))
